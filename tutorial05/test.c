...
static void test_parse_array() {
  ...
  for (i = 0; i < 4; ++i) {
    ...
    EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(p));
    EXPECT_EQ_SIZE_T(i, lept_get_array_size(p));
    for (j = 0; j < i; ++j) {
      EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(lept_get_array_element(p, j)));
      EXPECT_EQ_DOUBLE((double)j, lept_get_number(lept_get_array_element(p, j)));
    }
  }
  lept_free(&v);
}
